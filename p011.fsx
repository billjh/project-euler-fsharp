...
let leftRightProduct (n:int) (a:int[,]) = 
    ...
    |> Seq.collect (Seq.windowed n)

let upDownProduct (n:int) (a:int[,]) = 
    {0 .. Array2D.length2 a - 1} 
    |> Seq.map (fun c -> {0 .. Array2D.length1 a - 1} |> Seq.map (fun r -> a.[r, c])) 
    |> Seq.collect (Seq.windowed n)

let diagonalProduct (n:int) (a:int[,]) =
    {0 .. Array2D.length1 a - n}
    |> Seq.collect (fun r -> {0 .. Array2D.length2 a - n} |> Seq.map (fun c -> (r, c)))
    |> Seq.map(fun (r, c) -> {0 .. n - 1} |> Seq.map (fun offset -> a.[r + offset, c + offset]) |> Seq.toArray)

let reverseDiagonalProduct (n:int) (a:int[,]) = 
    {n - 1 .. Array2D.length1 a - 1}
    |> Seq.collect (fun r -> {0 .. Array2D.length2 a - n} |> Seq.map (fun c -> (r, c)))
    |> Seq.map(fun (r, c) -> {0 .. n - 1} |> Seq.map (fun offset -> a.[r - offset, c + offset]) |> Seq.toArray)
